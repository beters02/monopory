...
public sealed class MonopolyCommandWatcher : Component
{
	...
	{
		LogCommandResult( "buy_property", MonopolyCommandManager.BuyProperty( connection, propertyIndex, JoinPlayerName( playerName, playerNameTail ) ) );
	}
	...
	{
		LogCommandResult( "buy_property_set", MonopolyCommandManager.BuyPropertySet( connection, propertySet, JoinPlayerName( playerName, playerNameTail ) ) );
	}
	...
	private static void LogCommandResult( string commandName, MonopolyCommandResult result )
	{
		if ( result is null )
		{

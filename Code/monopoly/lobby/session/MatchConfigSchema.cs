...
public static class MatchConfigSchema
{
	...
	private static IReadOnlyList<MatchConfigOption> BuildOptions()
	{
		return new[]
		{
			...
			IntOption( "TurnTimeLimitSeconds", "Turn Rules", "Turn Time Limit Seconds", "How long each turn can last before timeout handling kicks in.", 33, 15, 900, 15, config => config.TurnTimeLimitSeconds, ( config, value ) => config.TurnTimeLimitSeconds = value ),
			IntOption( "InstantMoveButtonUnlockMinutes", "Turn Rules", "Instant Move Button Unlock Minutes", "Elapsed match minutes before the finish-movement button can appear. 0 allows it immediately.", 34, 0, 240, 5, config => config.InstantMoveButtonUnlockMinutes, ( config, value ) => config.InstantMoveButtonUnlockMinutes = value ),
